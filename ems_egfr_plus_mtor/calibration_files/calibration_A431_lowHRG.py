...
for il, dl, fl, c in zip(initial_lines, data_lines, final_lines, colors):
    ...
    dl.set_marker('x')
plt.show()
numpy.save('calibration_allpositions_A431_lowHRG.npy', mcmc.get_mixed_accepts(burn=opts.nsteps/10))
numpy.save('calibration_fittedparams_A431_lowHRG.npy', list(zip(opts.estimate_params, mcmc.cur_params()[mcmc.estimate_idx])))
