...
data_filename = os.path.join(os.path.abspath('../experimental_data'), 'experimental_data_H1666_highEGF.npy')
ydata_norm = numpy.load(data_filename)
var_data_filename = os.path.join(os.path.abspath('../experimental_data'), 'experimental_data_var_H1666_highEGF.npy')
...
for il, dl, fl, c in zip(initial_lines, data_lines, final_lines, colors):
    ...
    dl.set_marker('x')
plt.show()
numpy.save('calibration_allpositions_H1666_highEGF.npy', mcmc.get_mixed_accepts(burn=opts.nsteps/10))
numpy.save('calibration_fittedparams_H1666_highEGF.npy', list(zip(opts.estimate_params, mcmc.cur_params()[mcmc.estimate_idx])))
