...
obs_totals = [model.parameters['AKT_0'].value,
              model.parameters['erbb1_0'].value,
              model.parameters['ERK_0'].value]

# Load experimental data file and standard deviation file
os.chdir(os.path.dirname(os.path.abspath(__file__)))
data_filename = os.path.join(os.path.abspath('../experimental_data'), 'experimental_data_A431_highEGF_norm_splined.npy')
exp_data = np.load(data_filename)
var_data_filename = os.path.join(os.path.abspath('../experimental_data'), 'experimental_data_var_A431_highEGF_norm_splined.npy')
exp_sd = np.load(var_data_filename) #Standard deviation was calculated from the mean by assuming a coefficient of variation of .25; sdev's equal to 0 were set to 1 to avoid division by 0 errors.
...
tspan = np.linspace(0,7200, num=1000)

# Initialize solver object
solver = pysb.integrate.Solver(model, tspan, rtol=1e-3, atol=1e-6)
...
def objective_func(x, rate_mask, lb, ub):
    ...
    # Calculate error for trajectory comparisons
    ysim = solver.yobs
    ysim_array = extract_records(ysim, obs_names)
    ysim_norm = normalize(ysim_array)
    error = np.sum((exp_data - ysim_norm) ** 2 / (2 * exp_sd **2))
